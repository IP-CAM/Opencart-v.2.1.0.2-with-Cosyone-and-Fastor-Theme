...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "',
		price = '" . (float)$data['price'] . "',
		my_purchase_price = '" . (float)$data['my_purchase_price'] . "',
	...
	public function getProducts($data = array()) {
		$sql = "SELECT p.*, pd.*,  m.name as 'm_name' FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id)";
	...
	public function updateProductDefaultPrice($product_id, $default_price){
		$this->db->query("UPDATE " . DB_PREFIX . "product SET default_price = '" . floatval($default_price) . "' WHERE product_id = '" . (int)$product_id . "'");
	}

	public function getExpectedProfit(){
		$query = $this->db->query("SELECT SUM((price-my_purchase_price) * quantity) as expected_profit FROM oc_product WHERE 1");
		return $query->row['expected_profit'];
	}
}
